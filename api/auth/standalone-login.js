...
//
// 単体販売版（STANDALONE）専用のローカルログイン受け口（GET / POST を 1 関数に集約）。
//
// ── 流れ ──
//   1) ブラウザ（/login）が Supabase Auth（email/password）で signInWithPassword し、
//      取得した access_token（Supabase 発行の JWT）を本 API（POST）に渡す。
...
//   3) 検証に通ったら、既存の HMAC セッション基盤（issueSession）で media_session cookie を
...
export default async function handler(req, res) {
  ...
  // 既存 HMAC セッション基盤で media_session を発行（プラットフォーム版と同一機構＝callback.js と同じ）。
